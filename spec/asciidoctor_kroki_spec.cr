...
describe AsciicrystalKroki::Encoder do
  describe ".encode" do
    it "encodes diagram source text as deflate + base64url" do
      source = "A -> B: Hello"
      encoded = AsciicrystalKroki::Encoder.encode(source)
    ...
    it "produces deterministic output" do
      ...
      a = AsciicrystalKroki::Encoder.encode(source)
      b = AsciicrystalKroki::Encoder.encode(source)
      a.should eq(b)
    end
    ...
    it "round-trips through decode" do
      ...
      encoded = AsciicrystalKroki::Encoder.encode(source)
      decoded = AsciicrystalKroki::Encoder.decode(encoded)
      decoded.should eq(source)
    end

    it "handles empty string" do
      encoded = AsciicrystalKroki::Encoder.encode("")
      decoded = AsciicrystalKroki::Encoder.decode(encoded)
      decoded.should eq("")
    end
  ...
  describe ".build_url" do
    it "builds a correct Kroki URL for plantuml" do
      source = "A -> B"
      url = AsciicrystalKroki::Encoder.build_url("https://kroki.io", "plantuml", "svg", source)
    ...
    it "builds a correct URL for mermaid with png format" do
      source = "graph TD; A-->B;"
      url = AsciicrystalKroki::Encoder.build_url("https://kroki.io", "mermaid", "png", source)
      url.should start_with("https://kroki.io/mermaid/png/")
    end

    it "uses custom server URL" do
      source = "digraph { A -> B }"
      url = AsciicrystalKroki::Encoder.build_url("https://my-kroki.example.com", "graphviz", "svg", source)
      url.should start_with("https://my-kroki.example.com/graphviz/svg/")
    end

    it "builds URLs for all supported diagram types" do
      AsciicrystalKroki::DIAGRAM_TYPES.each do |diagram_type|
        url = AsciicrystalKroki::Encoder.build_url("https://kroki.io", diagram_type, "svg", "test")
...
describe AsciicrystalKroki do
  describe "KrokiTreeProcessor" do
    it "converts a plantuml block to an img tag" do
      ...
      html = AsciicrystalKroki.convert(input, {"safe" => "safe", "header_footer" => "false"})
    ...
    it "encodes the diagram source in the URL" do
      ...
      html = AsciicrystalKroki.convert(input, {"safe" => "safe", "header_footer" => "false"})
      expected_encoded = AsciicrystalKroki::Encoder.encode(source)
  ...
  describe "DIAGRAM_TYPES" do
    it "includes all expected diagram types" do
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("plantuml")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("mermaid")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("ditaa")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("graphviz")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("dot")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("d2")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("excalidraw")
      AsciicrystalKroki::DIAGRAM_TYPES.should contain("bpmn")
      AsciicrystalKroki::DIAGRAM_TYPES.size.should eq(27)
    end
  end
end
